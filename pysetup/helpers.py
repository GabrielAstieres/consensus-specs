...
def objects_to_spec(preset_name: str, spec_object: SpecObject, fork: str, ordered_class_objects: Dict[str, str]) -> str:
    """
    ...
    config_spec += "\n".join(
        f'    {k}: {v.type_name if v.type_name is not None else "int"}' for k, v in spec_object.config_vars.items()
    )
    ...
    hardcoded_func_dep_presets = reduce(
        lambda obj, builder: {**obj, **builder.hardcoded_func_dep_presets(spec_object)}, builders, {}
    )
    ...
    ssz_dep_constants_verification = "\n".join(
        map(lambda x: "assert %s == %s" % (x, spec_object.ssz_dep_constants[x]), filtered_ssz_dep_constants)
    )
