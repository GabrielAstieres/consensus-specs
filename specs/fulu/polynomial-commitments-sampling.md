        ...
        for j in range(len(ys)):
            if j != i:
                ...
                summand = multiply_polynomialcoeff(
                    summand,
                    PolynomialCoeff([-weight_adjustment * xs[j], weight_adjustment]),
                )
        r = add_polynomialcoeff(r, summand)
    return r
```
...
def compute_kzg_proof_multi_impl(
    ...
    return (
        KZGProof(
            g1_lincomb(
                KZG_SETUP_G1_MONOMIAL[: len(quotient_polynomial)], quotient_polynomial
            )
        ),
        ys,
    )
...
def verify_cell_kzg_proof_batch_impl(
    ...
    """
    assert (
        len(commitment_indices) == len(cell_indices) == len(cosets_evals) == len(proofs)
    )
    ...
    rli = bls.bytes48_to_G1(
        g1_lincomb(KZG_SETUP_G1_MONOMIAL[:n], sum_interp_polys_coeff)
    )
...
def verify_cell_kzg_proof_batch(
    ...
    assert (
        len(commitments_bytes) == len(cells) == len(proofs_bytes) == len(cell_indices)
    )
